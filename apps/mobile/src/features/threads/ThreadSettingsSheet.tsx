...
export function threadSettingsSummaryLabel(input: {
  ...
  // Providers that own their runtime (Pi) hide the runtime mode entirely.
  readonly showRuntimeModeToggle?: boolean;
}): string {
  const runtime =
    (input.showRuntimeModeToggle ?? true)
      ? RUNTIME_MODE_CHOICES.find((choice) => choice.mode === input.runtimeMode)
      : undefined;
  return [
    input.modelLabel,
      ...
      <View className="flex-1 justify-end">
        ...
        >
          ...
          <View style={{ paddingBottom: insets.bottom + 12 }}>
            ...
            {(props.showRuntimeModeToggle ?? true) ? (
              <DisclosureRow
                label="Runtime"
                value={
