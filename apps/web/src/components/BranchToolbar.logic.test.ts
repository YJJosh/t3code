...
describe("resolveEnvModeLabel", () => {
  it("uses explicit workspace labels", () => {
    ...
    expect(resolveEnvModeLabel("worktree")).toBe("New workspace");
  });
});
...
describe("resolveCurrentWorkspaceLabel", () => {
  ...
  it("describes the active checkout as a workspace when one is attached", () => {
    expect(resolveCurrentWorkspaceLabel("/repo/.t3/worktrees/feature-a")).toBe("Current workspace");
  });
});
...
describe("resolveLockedWorkspaceLabel", () => {
  ...
  it("uses a shorter label for an attached workspace", () => {
