...
describe("editable file language readiness", () => {
  it("cleans up an already terminated worker pool without leaving a scheduled broadcast", async () => {
    pool.terminate();
    await Promise.all(terminationPromises);
    ...
    try {
      ...
      // Drain the real Immediate queue after cleanup removes the browser globals.
