...
- **Project metadata** — shared link/copy rules are committed in `.workler`. The repository is initialized as a Workler project on first use (idempotent; it preserves the rules file, excludes `.worktrees/` from Git, and marks the repo with `workler.*` config).
