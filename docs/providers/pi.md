...
The composer settings control beside the model picker exposes the selected model's reasoning levels. When the active profile provides the compatible `effort-commands` extension, the same control also exposes context-window choices and Codex Fast service for supported OpenAI Codex models. T3 applies model, reasoning, context, and service-tier changes in order before sending the user prompt. Profile selection remains available there for new drafts.
